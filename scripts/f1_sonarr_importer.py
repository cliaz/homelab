...
def convert_path_between_host_and_container(path: Path, direction="host_to_container") -> Path:
    ...
    direction:
        ...
        - "container_to_host": maps a container path to host path
    """
    ...
    for mapping in PATH_MAPPINGS:
        ...
        if direction == "host_to_container" and path_str.startswith(host_path):
            converted_path = Path(container_path) / path_str[len(host_path):].lstrip("/")
            debug_print(f"Converted {path} -> {converted_path} ({direction})")
            return converted_path
        
        if direction == "container_to_host" and path_str.startswith(container_path):
            converted_path = Path(host_path) / path_str[len(container_path):].lstrip("/")
            debug_print(f"Converted {path} -> {converted_path} ({direction})")
            return converted_path
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "         Example: python3 f1_sonarr_importer.py\n"
            "         Example: python3 f1_sonarr_importer.py --path /downloads/f1/2025\n"
            ...
            "         Example (configure this in Qbittorrent Settings --> Downloads --> Run on torrent finished): /config/f1_sonarr_importer.py --category \"%L\" --path \"%F\"\n"
            "         (This assumes that your F1 downloads are tagged with the 'f1' category)\n"
